...
class TeamMemoryListCreateView(BaseMemoryViewSet, generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        has_access = (
            TeamMembership.objects.filter(user=user, team=team).exists() or
            team.organization.admin == user
        )
...
class OrganizationMemoryDetailView(BaseMemoryViewSet, generics.RetrieveUpdateDestroyAPIView):
    """
