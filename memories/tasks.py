...
def get_mem0_instance():
    ...
    if _mem0_instance is None:
        if settings.MEM0_API_KEY:
            raise ValueError("MEM0_API_KEY must be set in settings to create AsyncMemory instance")
...
def mem0_add_task(self, memory_type, pk, content):
    ...
    try:
        ...
        if result and "results" in result and len(result["results"]) > 0:
            ...
            logger.info(f"Successfully created mem0 memory for {memory_type} {pk}: {mem0_id}")
        else:
...
def mem0_update_task(self, memory_type, pk, mem0_id, content):
    ...
    try:
        ...
        logger.info(f"Successfully updated mem0 memory for {memory_type} {pk}: {mem0_id}")
...
def mem0_delete_task(self, memory_type, pk, mem0_id):
    ...
    try:
        ...
        logger.info(f"Successfully deleted mem0 memory for {memory_type} {pk}: {mem0_id}")
