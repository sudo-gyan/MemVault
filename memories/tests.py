...
class MemoryAPITest(APITestCase):
    ...
    def setUp(self):
        ...
        self.organization = Organization.objects.create(
            name="Test Org", admin=self.admin_user
        )

        self.team = Team.objects.create(
            name="Test Team", organization=self.organization
        )
        ...
        self.user_memory = UserMemory.objects.create(
            user=self.user1, content="User memory content"
        )

        self.team_memory = TeamMemory.objects.create(
            team=self.team, content="Team memory content"
        )

        self.org_memory = OrganizationMemory.objects.create(
            organization=self.organization, content="Organization memory content"
        )
    ...
    def test_org_admin_can_access_org_memories(self):
        ...
        self.assertEqual(len(response.data["results"]), 1)
        self.assertEqual(
            response.data["results"][0]["content"], "Organization memory content"
        )
    ...
    def test_create_team_memory(self):
        ...
        data = {"content": "New team memory", "team": self.team.id}
    ...
    def test_filtering_by_status(self):
        ...
        UserMemory.objects.create(
            user=self.user1, content="Completed memory", status="completed"
        )
    ...
    def test_search_in_content(self):
        ...
        UserMemory.objects.create(
            user=self.user1, content="This is a special memory about cats"
        )
